...
m3_speed_women <- speedglm(event ~ isled_standard + time + I(time^2) + male + in_union + enrol_mod + foreign_born + country_cohort - 1,
          data = analysis_proper_women,
          family = binomial(link = "logit"),
          weights = naive_perswgt)

# Model 4: Education × context + intercept by context + controls
m4_speed_women <- speedglm(event ~ isled_standard:country_cohort + time + I(time^2) + male + in_union +  enrol_mod + foreign_born + country_cohort - 1,
          data = analysis_proper_women,
          family = binomial(link = "logit"),
          weights = naive_perswgt)
```
